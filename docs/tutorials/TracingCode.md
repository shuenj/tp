...
Recall from the User Guide that the `edit` command has the format: `edit INDEX [n/NAME] [p/PHONE] [e/EMAIL] [a/ADDRESS] [af/AFFILIATION]…​` For this tutorial we will be issuing the command `edit 1 n/Alice Yeoh`.
...
1.  In this tutorial, we traced the "happy path" (i.e., no errors). What
    ...
    6.  `edit 1 af/one af/two af/three af/one`
